                ...
                <div className="space-y-6">
                    <div className="space-y-2">
                        <div className="flex justify-between">
                            ...
                            {user.role === 'admin' && (
                                <Button asChild className="my-auto">
                                    <Link href={onboarding.edit({ page: item.slug })}>Edit</Link>
                                </Button>
                            )}
                        </div>
                        <div className="flex gap-1">
                            ...
                            {user.role == 'admin' ? (
                                <div
                                    className={`rounded border px-2 py-1 text-xs ${item.published ? 'border-green-700 text-green-500' : 'border-yellow-500/60 text-yellow-500/80'}`}
                                >
                                    {item.published ? 'Published' : 'Drafted'}
                                </div>
                            ) : null}
                        </div>
                    </div>
