export default function SelectGender({
    ...
    value: 'Male' | 'Female';
    setValue: (data: 'Male' | 'Female') => void;
    disabled?: boolean;
}) {
    ...
    return (
        <div className="grid grid-cols-2 gap-2">
            <div
                onClick={() => !disabled && setValue('Male')}
                className={`${defaultClass} ${value == 'Male' ? maleClassSelected : maleClassDeselected}`}
            >
                Male
            </div>
            <div
                onClick={() => !disabled && setValue('Female')}
                className={`${defaultClass} ${value == 'Female' ? femaleClassSelected : femaleClassDeselected}`}
            >
                Female
            </div>
        </div>
